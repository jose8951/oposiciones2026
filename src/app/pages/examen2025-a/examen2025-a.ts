...
@Component({
  selector: 'app-examen2025-a',
  imports: [CommonModule],
  templateUrl: './examen2025-a.html',
  styleUrl: './examen2025-a.css',
})
export class Examen2025A implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Cambiamos el identificador a '2025a'
    this.examenService.obtenerExamen('2025a').subscribe({
      next: (resultado) => {
      ...
      error: (err) => {
        console.error('Error cargando el examen 2025A', err);
        this.cargando.set(false);
      },
    });
  }
